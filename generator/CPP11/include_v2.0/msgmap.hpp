...
public:
	...
	void operator<< (const std::array<T, _Size> &data);

	template<typename T>
	void operator>> (T &data);

	template<class T, size_t _Size>
	void operator>> (std::array<T, _Size> &data);
...
template<class T, size_t _Size>
void mavlink::MsgMap::operator<< (const std::array<T, _Size> &data)
{
	for (auto &v : data) {
		*this << v;
	}
...
template<class T, size_t _Size>
void mavlink::MsgMap::operator>> (std::array<T, _Size> &data)
{
	for (auto &v : data) {
		*this >> v;
	}
